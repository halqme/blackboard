...
func (s Store) Update(expectedRevision int, mutate func(*State) error) (State, error) {
	...
	defer db.Close()
	if err := s.initSchema(db); err != nil { return State{}, err }
	tx, err := db.Begin()
	if err != nil { return State{}, err }
	defer tx.Rollback()
	...
	if err != nil { return State{}, err }
	if changed != 1 { return State{}, ErrRevisionMismatch }
	st, err := loadStateTx(tx, s.ProjectID, expectedRevision)
	if err != nil { return State{}, err }
	if err := mutate(&st); err != nil { return State{}, err }
	st.ProjectID = s.ProjectID
	st.Revision = expectedRevision + 1
	if err := syncStateTx(tx, st); err != nil { return State{}, err }
...
func loadStateTx(tx *sql.Tx, projectID string, revision int) (State, error) {
	...
	for rows.Next() {
		var t Task
		if err := rows.Scan(&t.ID, &t.Title, &t.Stage, &t.Status, &t.CreatedAt, &t.UpdatedAt); err != nil { return State{}, err }
		st.Tasks = append(st.Tasks, t)
	}
	if err := rows.Err(); err != nil { return State{}, err }
	...
	for artifacts.Next() {
		var a Artifact
		if err := artifacts.Scan(&a.ID, &a.TaskID, &a.Stage, &a.Kind, &a.Version, &a.Status, &a.BlobHash, &a.BasedOnRevision, &a.CreatedAt); err != nil { return State{}, err }
		st.Artifacts = append(st.Artifacts, a)
	}
	if err := artifacts.Err(); err != nil { return State{}, err }
