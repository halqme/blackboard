...
// Update applies mutate to the latest persisted state only when expectedRevision
// still matches the database revision. The revision check, mutation, state sync,
// and revision increment are committed atomically in one SQLite transaction.
func (s Store) Update(expectedRevision int, mutate func(*State) error) (State, error) {
	...
	result, err := tx.Exec(
		...
		strconv.Itoa(expectedRevision+1),
